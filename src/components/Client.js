...
export default class Sync extends Component {
    ...
    */
    ...
    inject = async (signature, data) => {
        axios.post('http://localhost:5000/forge/sign', {
            op: data.operation,
            sig: signature,
    ...
    requestOperation = async () => {
        ...
        const client = new DAppClient({ name: 'ungrund' })
        const operationResponse = await client.requestOperation({
            operationDetails: [
                {
    ...
    contractOrigination = async (client) => {
        axios.post('http://localhost:5000/forge/origination', { tz: this.state.address })
            .then(async resp => {
